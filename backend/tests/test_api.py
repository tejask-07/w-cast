...
from app.services.gfs_forecast_service import map_api_variable_to_gfs, select_gfs_cycle
...
def test_select_gfs_cycle_uses_latest_completed_utc_cycle():
    ...
    assert select_gfs_cycle(now) == datetime(2026, 9, 22, 6, 0, tzinfo=timezone.utc)
...
def test_gfs_adapter_uses_mocked_subset_data(monkeypatch):
    ...
    def fake_download(date, forecast_hour, save_dir):
        ...
        return {'temperature': 't.grib2', 'wind_u': 'u.grib2', 'wind_v': 'v.grib2', 'precipitation': 'p.grib2'}

    def fake_extract(file_path, lat, lon, lead_hours):
        assert file_path == {'temperature': 't.grib2', 'wind_u': 'u.grib2', 'wind_v': 'v.grib2', 'precipitation': 'p.grib2'}
        assert lat == 19.07
        assert lon == 72.87
    ...
    monkeypatch.setattr(gfs_forecast_service, 'download_gfs_subsets', fake_download)
    monkeypatch.setattr(gfs_forecast_service, 'extract_gfs_point', fake_extract)

    result = gfs_forecast_service.generate_real_gfs_forecast(19.07, 72.87, 24, 'rainfall')
    assert result == {'temperature': 26.5, 'rainfall': 12.8, 'wind_speed': 18.7}
...
def test_gfs_adapter_failure_becomes_service_error(monkeypatch):
    ...
    monkeypatch.setattr(gfs_forecast_service, 'download_gfs_subsets', fake_download)
    try:
        gfs_forecast_service.generate_real_gfs_forecast(19.07, 72.87, 24, 'rainfall')
...
def test_forecast_valid_request(monkeypatch):
    ...
    def fake_generate_real_gfs_forecast(**kwargs):
        return {
            'temperature': 26.5,
            'rainfall': 12.8,
            'wind_speed': 18.7,
    ...
    monkeypatch.setattr(forecast_service, 'generate_real_gfs_forecast', fake_generate_real_gfs_forecast)
